...
export default function FatawaCategoriesPage() {
  ...
  async function fetchCategories() {
    const res = await fetch("/api/fatawa-categories");
  ...
  async function handleDelete(id: number, catName: string) {
    const confirmed = await confirm({
      title: "حذف التصنيف",
      message: `هل أنت متأكد من حذف تصنيف "${catName}"؟ سيؤثر ذلك على الفتاوى المرتبطة به.`,
      confirmLabel: "نعم، احذف",
      variant: "danger",
    });
    if (!confirmed) return;
    const res = await fetch(`/api/fatawa-categories/${id}`, { method: "DELETE" });
    if (res.ok) {
    ...
    <div className="p-8 max-w-5xl" dir="rtl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold text-gray-800">تصنيفات الفتاوى</h1>
          </div>
      ...
      {showForm && (
        <div className="bg-white border border-green-100 rounded-2xl p-5 mb-4 shadow-sm">
          <h2 className="text-sm font-medium text-gray-700 mb-4">إضافة تصنيف جديد</h2>
          <div className="space-y-3">
            <div>
              ...
              <input
                type="text"
                placeholder="مثال: الطهارة والصلاة"
                value={name}
