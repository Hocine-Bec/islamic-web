...
async function HeroStats() {
  const [postsCount, categories, commentsCount] = await Promise.all([
    getPostsCount(),
    getAllCategories(),
    getCommentsCount(),
  ]);
  ...
  return (
    <div className="flex justify-center">
      <div className="flex items-center divide-x divide-x-reverse divide-white/20">
        {[
          { num: postsCount, label: "درساً ومحاضرة" },
          { num: categories.length, label: "تصنيفاً علمياً" },
          { num: commentsCount, label: "مشاركة وتعليق" },
        ].map((s) => (
...
async function HomeLatestContent() {
      ...
      <main className="flex-1">
        ...
        <section className="bg-green-700 relative overflow-hidden">
          ...
          <div className="relative max-w-5xl mx-auto px-6 py-12 text-center">
            ...
            <Suspense fallback={<div className="h-10 w-64 bg-white/10 rounded-full mx-auto animate-pulse" />}>
              <HeroStats />
            </Suspense>
          </div>
        </section>

        <Suspense fallback={<div className="max-w-5xl mx-auto px-6 py-20 animate-pulse bg-gray-100 rounded-3xl mt-10 h-96" />}>
          <HomeLatestContent />
        </Suspense>
